...
async function toggleLike(blogId, iconElement) {
  try {
    ...
    // Send request to backend API to toggle like status
    const response = await fetch(
      `http://localhost:5000/api/blog/${blogId}/like`,
      {
        method: "POST", // Assuming POST request to toggle like
        headers: {
          Authorization: `Bearer ${token}`,
        },
      }
    ...
    if (response.status === 200) {
      // Liked successfully, increment like count and change icon color
      likeHolder.textContent = likeCount + 1;
      iconElement.classList.add("liked"); // Add class to change color
    } else if (response.status === 204) {
      // Unliked successfully, decrement like count and change icon color
      likeHolder.textContent = likeCount - 1;
      iconElement.classList.remove("liked"); // Remove class to revert color
    }
  } catch (error) {
